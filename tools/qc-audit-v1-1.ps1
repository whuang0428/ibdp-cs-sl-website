...
Write-Host "=== IBDP CS SL Website QC Audit v1.1 ===" -ForegroundColor Cyan
...
Write-Host "`n--- Checking Markdown local links ---" -ForegroundColor Cyan
...
foreach ($file in $mdFiles) {
    ...
    foreach ($lm in $linkMatches) {
        ...
        if ($cleanTarget.StartsWith("/")) {
            ...
            if ($null -ne $routeFile) { $candidatePaths += $routeFile }
        } else {
...
Write-Host "`n--- Checking common placeholders and local-only text ---" -ForegroundColor Cyan
$patterns = @("TODO", "FIXME", "D:\\", "C:\\", "/mnt/data", "file://", "api_key", "secret", "token")
foreach ($pattern in $patterns) {
    $hits = Select-String -Path (Join-Path $docs "**\*.md") -Pattern $pattern -CaseSensitive:$false -ErrorAction SilentlyContinue
    foreach ($hit in $hits) {
        Add-Warning "Pattern '$pattern' found in $($hit.Path): line $($hit.LineNumber)"
    }
}

Write-Host "`n--- Checking recommended page sections ---" -ForegroundColor Cyan
$recommended = @("Lesson Goals", "Teacher Notes", "One-page")
foreach ($file in $mdFiles) {
    $content = Get-Content $file.FullName -Raw -Encoding UTF8
    if ($file.FullName -match "\\(a1|a2|a3|a4|b1|b2|extension|ia-support|glossary)\\") {
        foreach ($section in $recommended) {
            if ($content -notmatch [regex]::Escape($section)) {
                Add-Warning "Recommended section '$section' not found in $($file.FullName)"
            }
        }
    }
}
